/*
  ...
  1. New Tables
    ...
    - `user_organizations` - Many-to-many relationship for users and orgs
    ...
    - `financial_records` - Financial tracking (invoices, payments, etc.)
  ...
  3. Functions
    ...
    - Trigger functions for activity logging and timestamps
*/
...
CREATE POLICY "Users can view organizations they belong to" ON organizations FOR SELECT TO authenticated 
USING (id IN (SELECT organization_id FROM user_organizations WHERE user_id = uid()));

CREATE POLICY "Organization admins can update organizations" ON organizations FOR UPDATE TO authenticated 
USING (id IN (SELECT organization_id FROM user_organizations WHERE user_id = uid() AND role IN ('owner', 'admin')));

-- Projects policies
CREATE POLICY "Users can view projects in their organizations" ON projects FOR SELECT TO authenticated 
USING (
...
CREATE POLICY "Users can create projects in their organizations" ON projects FOR INSERT TO authenticated 
WITH CHECK (user_has_org_access(organization_id) AND created_by = uid());

CREATE POLICY "Project managers can update their projects" ON projects FOR UPDATE TO authenticated 
USING (project_manager_id = uid() OR created_by = uid() OR get_user_role() = 'admin');

-- Tasks policies
CREATE POLICY "Users can view tasks in accessible projects" ON tasks FOR SELECT TO authenticated 
USING (
  project_id IN (
...
CREATE POLICY "Users can create tasks in accessible projects" ON tasks FOR INSERT TO authenticated 
WITH CHECK (
  project_id IN (
    SELECT id FROM projects WHERE 
    organization_id IN (SELECT organization_id FROM user_organizations WHERE user_id = uid()) OR
...
-- Files policies
CREATE POLICY "Users can view accessible files" ON files FOR SELECT TO authenticated 
USING (
  uploaded_by = uid() OR
  is_public = true OR
  organization_id IN (SELECT organization_id FROM user_organizations WHERE user_id = uid()) OR
...
CREATE POLICY "Users can upload files" ON files FOR INSERT TO authenticated 
WITH CHECK (uploaded_by = uid() AND (organization_id IS NULL OR user_has_org_access(organization_id)));

CREATE POLICY "Users can update own files or with permissions" ON files FOR UPDATE TO authenticated 
USING (
  uploaded_by = uid() OR
...
CREATE POLICY "Users can view own notifications" ON notifications FOR SELECT TO authenticated USING (user_id = uid());
CREATE POLICY "Users can update own notifications" ON notifications FOR UPDATE TO authenticated USING (user_id = uid());
